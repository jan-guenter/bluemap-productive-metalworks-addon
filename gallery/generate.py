...
"""Generate the bounded deterministic Productive Metalworks gallery."""
...
def placements_tsv() -> bytes:
    lines = ["case_id\tlabel\tx\ty\tz\tblock_state\tblock_nbt\texpected"]
    lines.extend(
        "\t".join(
            (
                placement.case_id,
    ...
    for placement in cases.PLACEMENTS:
        lines.extend(
            (
                ...
                f"{placement.block_state}{placement.block_nbt}",
            )
        )
...
def verify_function() -> bytes:
    ...
    lines.append(
        "tellraw @a {\"text\":\"Productive Metalworks gallery checked\","
        "\"color\":\"green\"}"
    )
    return text_bytes("\n".join(lines))
...
def base_files() -> dict[Path, bytes]:
    ...
    return {
        ...
        Path("datapack/pack.mcmeta"): json_bytes(
            {
                "pack": {
                    "description": "ATM 1.2.0 Productive Metalworks render gallery",
                    "pack_format": 48,
                }
            }
        ),
        ...
        function_root / "load.mcfunction": text_bytes(
            "# Generated by gallery/generate.py; do not edit.\n"
            "tellraw @a {\"text\":\"Productive Metalworks gallery loaded; "
